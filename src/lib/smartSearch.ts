 ...
 *
 * Usage:
 *   import { smartSearch, createSearcher } from '@/lib/smartSearch';
 *
 *   // simple one-off
 *   const results = smartSearch(items, 'base max glow', { keys: ['name','id'] });
 *
 *   // reusable preprocessed searcher for many queries
 *   const searcher = createSearcher(items, { keys: ['name','id'], nameKey: 'name' });
...
function scoreTokens(qTokens: string[], itemTokens: string[]): number {
  ...
  for (const q of qTokens) {
    if (!q) continue;
    let best = { score: 0, kind: null as string | null };
 ...
 */
function preprocessItem(item: any, keys: string[] = ['name']) {
  const pre = { original: item, fields: {} as Record<string, { raw: string; norm: string; tokens: string[] }>, combinedTokens: [] as string[] };
  for (const k of keys) {
    const raw = item[k] || '';
 ...
 */
export function smartSearch(items: any[] = [], query: string = '', options: SmartSearchOptions = {}): SearchResult[] {
  ...
  for (const it of items) {
    ...
    const combined = Object.values(pre.fields).map((f: any) => f.norm).join(' ');
    let bonus = 0;
...
interface Searcher {
  search: (query: string, opts?: SmartSearchOptions) => SearchResult[];
}
 ...
 */
export function createSearcher(items: any[] = [], options: SmartSearchOptions = {}): Searcher {
  const keys = options.keys || ['name'];
  const nameKey = options.nameKey || 'name';
  ...
  function search(query: string, opts: SmartSearchOptions = {}): SearchResult[] {
